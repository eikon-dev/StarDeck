...
2) Edit Types (UX correctness)
3) UI polish (readability & focus)
4) StarField (visual reward layer)

## 0) Кратко

**Цель v2:** усилить базовый трекер задач и добавить “звёздное небо” как постоянный визуальный прогресс, параллельно
подняв качество проекта до уровня “готовится к прод”.
**Ключевой результат:** задачи удобнее вести каждый день, прогресс виден не только цифрами, но и в фоне (небо), кодовая
база становится стабильнее (тесты + закрытие долгов).
...
- [X] Отображение priority аккуратнее
    - [X] Единый компонент Badge для priority
...
- [ ] Упростить зависимости в местах, где “течёт” (TODOs v1)
    - [X] TODO в fx.type.ts
    - [X] TODO в FXSystem.ts
    - [X] TODO в useTasksStore.ts
    - [ ] TODO в selectTasksSorts.ts
...
- [X] resetDailyCycle работает по правилам и не ломает награды
